...
def ssne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, showProcess=False):
    """
    ...
    for iter in range(max_iter):
        ...
        if showProcess and (iter + 1) % 100 == 0:
            ...
            pylab.savefig(os.path.join('ssne_results','ssne_iter_{}.png'.format(iter + 1)))
...
def plotSimilarity(P, Q, perplexity=20):
    ...
    pylab.savefig(os.path.join('ssne_results', 'ssne_high_D_similarity.png'))
    
    pylab.clf()
    pylab.title('Low-dimensional pairwise similarity, perplexity = {}'.format(perplexity))
    pylab.hist(Q.flatten(), bins=100, log=True)
    pylab.savefig(os.path.join('ssne_results', 'ssne_low_D_similarity.png'))
...
if __name__ == "__main__":
    ...
    '''
    Y, P, Q = ssne(X, 2, 50, 20.0, True)

    # show the final result
    pylab.clf()
    pylab.scatter(Y[:, 0], Y[:, 1], 20, labels)
    pylab.savefig(os.path.join('ssne_results','ssne_iter_final.png'))
    pylab.show()

    # show the pairwise similarities
    plotSimilarity(P, Q)
    '''
    ...
    for i in candidates:
        ...
        pylab.clf()
        pylab.scatter(Y[:, 0], Y[:, 1], 20, labels)
        pylab.savefig(os.path.join('ssne_results','ssne_perp_{}_final.png'.format(int(i))))